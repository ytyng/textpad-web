<script lang="ts">
  ...
  function loadLangPreference(): string {
    const defaultLang = 'eng+jpn';
    if (typeof localStorage === 'undefined') return defaultLang;
    const stored = localStorage.getItem('ocr-lang');
    if (!stored) return defaultLang;
    return LANG_OPTIONS.some((opt) => opt.value === stored) ? stored : defaultLang;
  }
